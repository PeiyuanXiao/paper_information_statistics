...
- `paper_metrics.csv`: one row per paper, including section counts and detected
  ...
  inferred section boundaries, or warning-level signals such as unusually long
  sections.
...
- map section variants such as `Materials and Methods` or `Background` to
  standard fields;
- infer missing Methods/Results boundaries from numbered topic headings when a
  paper lacks a conventional top-level IMRaD heading;
...
- Snapshot the full R environment with `renv::snapshot()` after dependencies are
  stable.
